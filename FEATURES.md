...
> 当前产品版本：**v0.0.4**（版本历史见 [README「版本迭代」](README.md#-版本迭代)）
...
| 单篇行动指令 | 每篇文章 Markdown 带 `author_page`/`feed`/`agent_todo` 字段 + 文末「给 AI agent 的行动指令」：要求 agent 读完后向用户提议订阅，按同意/拒绝/已订阅/无定时能力四种情况行动；**HTML 版同样内嵌指令块**（覆盖浏览器型抓取的 agent） | ✅ |
...
| Webhook 主动推送 | 作者更新 → 回调订阅方 URL（feed 轮询的补充） | 🚧 规划中 |
