...
用法:
  ...
  a4a renew <id>                      把带有效期的旧文章转为永久（历史兼容）
  a4a delete <id>                     删除文章
...
function printPublished(result, values) {
  ...
  if (result.expiresAt) console.log(`有效期至 ${result.expiresAt.slice(0, 10)}`)
  else if (result.admin_url) console.log(`链接永久有效（管理后台: ${result.admin_url}）`)
  if (result.commands) {
